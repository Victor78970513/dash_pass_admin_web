...
class EditTollPage extends StatefulWidget {
  final TollModel toll;
  static const name = "edit-toll";
  const EditTollPage({super.key, required this.toll});

  @override
  State<EditTollPage> createState() => _EditTollPagePageState();
}

class _EditTollPagePageState extends State<EditTollPage> {
  ...
  @override
  void initState() {
    nameController.text = widget.toll.name;
    for (int i = 0; i < widget.toll.tarifas.length; i++) {
      selectedVehiclesPerTariff[i + 1] =
          widget.toll.tarifas[i].clasificacion.toSet();
      tarifas.add(
        TarifasWidget(
          controller: TextEditingController(
              text: widget.toll.tarifas[i].monto.toString()),
          title: "Clasificacion ${i + 1}",
          onSelectedVehiclesChanged: (selectedVehicles) {
            selectedVehiclesPerTariff[i + 1] = selectedVehicles;
          },
          olderVehicles: widget.toll.tarifas[i].clasificacion,
        ),
      );
    }
    super.initState();
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
            create: (context) =>
                AdminUidCubit()..changeAdminUid(widget.toll.adminId)),
        BlocProvider(
                                ...
                                GestureDetector(
                                  onTap: () async {
                                    for (int i = 0; i < tarifas.length; i++) {
                                      ...
                                      print(
                                          "-------------------------------------------");
                                      context.read<TollsCubit>().editToll(
